...
| **Аудитория** | Product, разработка, дизайн |

**Связанные документы:**

- [PROEKT.md](./PROEKT.md) — стек, решения, архитектура, тестовые данные
- [etapy/README.md](./etapy/README.md) — этапы разработки (7 файлов)
- [openapi.yaml](../openapi.yaml) — REST API (расширенный контракт)
- Приём форм с Tilda: **`GET|POST /ingest/seolead`** (см. PROEKT §5); обратная совместимость с `crm.wbooster.ru` **не требуется**
...
- **Навигация:**
  - **Лиды** — общий список по всем доступным проектам заказчика (фильтр по сайту).
  - **По проекту** — для каждого сайта два раздела:
    - **Лиды и продажи** — список лидов с фиксированным `site_id`;
    - **Трафик и лиды** — заглушка в MVP; отчёт по таблице — **v2** (см. [§6.4](#64-отчёт-трафик-и-лиды-v2)).
...
### 6.4. Отчёт «Трафик и лиды» (v2)

Раздел ЛК: `/cabinet/projects/{siteId}/traffic` — сводная таблица по проекту (аналог отчёта в legacy WBooster).

#### 6.4.1. Назначение

Заказчик видит **сквозную картину**: рекламный трафик + лиды за период, с расчётными KPI (CTR, CV, CPA). Два уровня детализации:

| Режим | Группировка | Пример строки |
|-------|-------------|---------------|
| По месяцам | `YYYY-MM` | Май '26, Апр '26 |
| По дням | `YYYY-MM-DD` | 2026-05-25 |

UI: фильтр по тексту, выбор «показать N записей», пагинация, сортировка по колонкам.

#### 6.4.2. Колонки таблицы

| Колонка (UI) | Код | Источник | Примечание |
|--------------|-----|----------|------------|
| Месяц / Дата | `period` | группировка | Месяц или день |
| Показы | `impressions` | **Яндекс Метрика** Reporting API | При связке счётчика с Директом — рекламные метрики |
| Клики | `clicks` | **Яндекс Метрика** Reporting API | |
| CTR, % | `ctr` | вычисляемое | `clicks / impressions × 100` |
| CPC avg | `cpc_avg` | вычисляемое | `cost / clicks` |
| Расход | `cost` | **Яндекс Метрика** Reporting API | Сумма расходов по рекламе за период |
| Звонки | `calls_count` | **CRM** (`leads`, `channel=call`) | Агрегация по `site_id` и дате |
| Заявки | `forms_count` | **CRM** (`leads`, `channel=form`, `email`) | |
| Сумма лидов | `leads_total` | **CRM** | `calls_count + forms_count` |
| CV, % | `conversion_rate` | вычисляемое | `leads_total / clicks × 100` |
| CPA | `cpa` | вычисляемое | `cost / leads_total` |

Если рекламные метрики недоступны (счётчик не связан с Директом, нет OAuth) — колонки трафика показывают `0` или «—»; колонки лидов заполняются из CRM.
...
flowchart LR
  ...
  subgraph metrika [Яндекс Метрика]
    Reporting["Reporting API"]
  end
  Site["site.metrika_counter_id"]
  Site --> Reporting
  Reporting -->|"показы, клики, расход"| Stats["traffic_daily_stats"]
  Leads -->|"звонки, заявки"| Stats
  Stats --> API["GET /api/client/projects/{siteId}/traffic-stats"]
  API --> LK["ЛК: Трафик и лиды"]
```

| Данные | Откуда | Статус |
|--------|--------|--------|
| Лиды по каналам | Таблица `leads`, фильтр `site_id` + период | Реализуемо сразу (агрегация SQL) |
| Показы, клики, расход | **Reporting API Метрики** по `metrika_counter_id` проекта | **План v2** (основной источник) |
| OAuth / токен | Общий с обогащением лидов (`METRIKA_REPORTING_ENABLED`, `METRIKA_OAUTH_TOKEN`) | Частично реализовано для атрибуции одного лида |

**Решение по умолчанию:** рекламный блок тянуть из **Reporting API Яндекс Метрики**, не из отдельного API Директа (v1 отчёта). Отдельная интеграция с API Директа — опционально, если метриковских метрик недостаточно.

Запросы к Метрике — **агрегаты по дням/месяцам** (dimensions: `ym:s:date` или `ym:s:startOfMonth`; metrics: визиты, клики, расход — уточнить по [документации Reporting API](https://yandex.ru/dev/metrika/doc/api2/api_v1/intro.html) при реализации). Не путать с текущим запросом по `clientID` одного лида (§9.5).

#### 6.4.4. API и кэш (план реализации)

| Компонент | Описание |
|-----------|----------|
| `GET /api/client/projects/{siteId}/traffic-stats` | Параметры: `group_by=day\|month`, `date_from`, `date_to`, пагинация |
| Таблица `traffic_daily_stats` (или аналог) | Кэш снимков по `(site_id, date)` — обновление по cron, чтобы не дергать Метрику при каждом открытии ЛК |
| Job `SyncTrafficStatsFromMetrikaJob` | Ежедневная/почасовая синхронизация для активных проектов с `metrika_counter_id` |
| Права | Тот же scope, что у лидов: `ClientSiteAccess`, только доступные `site_id` |

#### 6.4.5. Этапы внедрения

| Этап | Содержание | Результат |
|------|------------|-----------|
| **6.4.A** | UI таблицы + агрегация лидов из CRM | Колонки: дата, звонки, заявки, сумма лидов |
| **6.4.B** | Reporting API: показы, клики, расход + merge | Полная таблица как в legacy WBooster |
| **6.4.C** | Кэш, cron, обработка ошибок OAuth | Стабильная работа на prod |

#### 6.4.6. User story

9. Как пользователь ЛК, я открываю **«Трафик и лиды»** по проекту и вижу помесячную/подневную сводку рекламы и лидов, чтобы оценить эффективность кампаний.

### 6.5. Аналитика — графики (v2)

Раздел ЛК: `/cabinet/projects/{siteId}/analytics` — дашборд с виджетами (аналог legacy WBooster).

#### 6.5.1. Виджеты

| Виджет | Графики | Reporting API |
|--------|---------|---------------|
| Переходы из поисковых систем | stacked area + donut | `ym:s:searchEngine`, фильтр organic |
| Поисковый брендированный трафик | area + donut | фильтр `ym:s:searchPhrase=~'(brand…)'` + organic |
| Поисковый небрендированный трафик | area + donut | фильтр `ym:s:searchPhrase!~'(brand…)'` + organic |
| Распределение трафика по каналам | donut + таблица | `ym:s:trafficSource` |
| География посетителей | donut | `ym:s:regionCity` |
| Тип устройств | donut + таблица | `ym:s:deviceCategory` |

Таблица (где есть): визиты, отказы %, глубина, время (MM:SS).

#### 6.5.2. API

Префикс: `GET /api/client/projects/{siteId}/analytics/*`

| Endpoint | Отчёт |
|----------|-------|
| `traffic-sources` | Каналы |
| `search-engines` | Поисковики |
| `search-branded` | Брендовый поиск |
| `search-non-branded` | Небрендовый поиск |
| `geography` | География |
| `devices` | Устройства |

Query: `date_from`, `date_to`, `group_by=day|month`, `refresh=1` (сброс кэша).

Кэш: таблица `metrika_report_cache`, TTL 4 часа.

#### 6.5.3. User story

10. Как пользователь ЛК, я открываю **«Аналитика»** по проекту и вижу графики трафика из Метрики, чтобы понимать структуру аудитории и источники.
...
### 8.1. `POST /api/v1/leads` (и алиас логики для `/ingest/seolead`)
...
### 9.4. Reporting API — обогащение лида (реализовано опционально)

При `METRIKA_REPORTING_ENABLED=true` и OAuth-токене job `EnrichLeadFromMetrikaJob` после ingest запрашивает **атрибуцию одного визита** по `metrika_client_id` за день лида (источник трафика, UTM). См. [metrika-kanal-i-crm-lead.md](./metrika-kanal-i-crm-lead.md).

### 9.5. Reporting API — агрегаты для отчёта «Трафик и лиды» (v2)

Отдельный сценарий от §9.4: **сводная статистика по счётчику проекта**, без фильтра по Client ID.

| Параметр | Значение |
|----------|----------|
| Счётчик | `site.metrika_counter_id` |
| Период | `date_from` … `date_to` (как в фильтре ЛК) |
| Группировка | день (`ym:s:date`) или месяц |
| Метрики (целевые) | показы, клики, расход — через метрики Метрики, связанные с рекламой (уточнить при разработке; зависит от связки счётчика с Яндекс Директом) |
| Хранение | кэш в БД + периодический sync (§6.4.4) |

Лиды за тот же период считаются в CRM и **склеиваются по дате** с данными Метрики. Расчётные поля CTR, CV, CPA — на backend или frontend из объединённой строки.

### 9.6. Валидация домена
...
|---|--------|---------------------------|
| 1 | Обогащение UTM / источника трафика по Client ID (Reporting API) | Реализовано опционально; см. §9.4 |
...
| 6 | Отчёт «Трафик и лиды» в ЛК | **v2**; трафик — Reporting API Метрики (§6.4, §9.5); лиды — CRM |
